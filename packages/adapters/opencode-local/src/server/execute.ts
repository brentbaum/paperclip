...
function buildTelegramWakePrompt(context: Record<string, unknown>, agentId: string) {
  ...
  return [
    ...
    "",
    "IMPORTANT: You MUST reply via the Paperclip API. Do NOT use any other telegram skill, script, or tool (e.g. telegram-notify, telegram_notify.py). Those are disabled and will send to the wrong chat.",
    "",
    `Endpoint: POST $PAPERCLIP_API_URL${sendEndpoint}`,
    `Headers: Authorization: Bearer $PAPERCLIP_API_KEY, Content-Type: application/json`,
    `Body: ${JSON.stringify(replyPayload)}`,
    "",
    "IMPORTANT: Use Python (urllib or requests) to call this endpoint — do NOT use curl, as shell escaping corrupts the JSON when text contains apostrophes or special characters.",
...
export async function execute(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
  ...
  if (wakeMessage) env.PAPERCLIP_WAKE_MESSAGE = wakeMessage;
